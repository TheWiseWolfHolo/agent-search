...
const wrapperPath = path.resolve(__dirname, "..", "bin", "agent-search.js");
...
function runWrapper({ runtimeExists, repairStatus = 0, repairCreatesRuntime = true }) {
  ...
  const fakeFs = {
    existsSync(filePath) {
      ...
      if (
        normalized.endsWith("/.agent-search-python/Scripts/python.exe") ||
        normalized.endsWith("/.agent-search-python/bin/python")
      ) {
...
assert.deepStrictEqual(Array.from(repaired.spawnCalls[0].args.slice(0, 2)), ["-m", "agent_search.cli"]);
