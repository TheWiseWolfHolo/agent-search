...
PUBLIC_SKILL_DIR = ROOT / "skills" / "agent-search-cli"
PACKAGED_SKILL_DIR = ROOT / "src" / "agent_search" / "assets" / "skills" / "agent-search-cli"
...
def test_regression_does_not_create_repo_log_file():
    ...
    if not log_dir.exists():
        return
    assert not list(log_dir.glob("agent_search_*.log"))


def test_agent_search_skill_contract_enforces_cli_first():
    skill_dir = Path.home() / ".codex" / "skills" / "agent-search-cli"
...
def test_deep_research_skill_contract_public_and_packaged_assets_match():
    ...
    required_markers = [
        ...
        "gap_check",
        "fetch_before_claim",
        "agent-search skills status",
        "agent-search skills update",
        ...
        "fixed topic recipe",
        "深度搜索一下最近的比特币行情",
        "C:\\tmp\\agent-search-evidence",
        "mock-full plus live-limited",
        "public planner entrypoint",
        "not an executor",
        "does not change default `agent-search search`",
...
def test_deep_research_cli_contract_documents_plan_and_smoke_matrix():
    ...
    required_markers = [
        ...
        "`agent-search deep` is the public offline planner command",
        "must not change default `agent-search search` behavior",
...
def test_search_timeout_retry_policy_is_distributable():
    ...
    skill_markers = [
        ...
        "Do not wrap `agent-search` in a shell-level `timeout` command",
        "Do not rely on `AGENT_SEARCH_RETRY_*` settings",
    ...
    contract_markers = [
        ...
        "`agent-search search ... --timeout 180 --extra-sources 1 --format json --output PATH`",
        "not a shell-level `timeout` wrapper",
        "`AGENT_SEARCH_RETRY_*` settings are not the contract",
...
def test_web_source_reinforcement_contract_public_and_packaged_assets_match():
    ...
    contract_markers = [
        ...
        "Tavily first, then Firecrawl",
        "same capability",
        "Firecrawl currently reports whether `FIRECRAWL_API_KEY` is configured",
    ]
    skill_markers = [
        "TAVILY_API_URL",
        "FIRECRAWL_API_URL",
        "Tavily -> Firecrawl",
        "doctor` tests configured main-search providers, Exa, Tavily, and Context7 connectivity",
    ]
