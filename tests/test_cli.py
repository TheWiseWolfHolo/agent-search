...
from agent_search import skill_installer
...
def test_version_flags_exit_successfully(monkeypatch, capsys):
    ...
    for flag in ["--version", "--v", "-v"]:
        ...
        assert capsys.readouterr().out.strip() == "agent-search 9.9.9-test"
...
def test_each_subcommand_help_exits_successfully(capsys):
    ...
    out = capsys.readouterr().out
    assert "usage: agent-search search" in out
    assert "usage: agent-search regression" in out
...
def test_diagnose_openai_compatible_defaults_to_markdown(monkeypatch, capsys):
    async def fake_diagnose(timeout_seconds=30.0):
        return {
            ...
            "next_command": "agent-search diagnose openai-compatible --format markdown",
    ...
    assert out.startswith("# WolfHolo Agent Search Diagnose")
    assert "小请求能通" in out
    assert "真实 search 请求" in out
    assert "agent-search diagnose openai-compatible --format markdown" in out
...
def test_search_outputs_json_and_file(monkeypatch, capsys):
    ...
    output = "C:/tmp/agent-search-cli-test-result.json"
...
def test_doctor_markdown_outputs_human_health_report(monkeypatch, capsys):
    ...
    async def fake_doctor():
        return {
            ...
            "config_dir": "C:/tmp",
            "config_dir_source": "environment",
            "default_config_file": "C:/Users/example/AppData/Local/agent-search/config.json",
            "legacy_windows_config_file": "C:/Users/example/.config/agent-search/config.json",
            "legacy_windows_config_exists": True,
            "config_dir_override_value": "C:/Users/example/AppData/Local/agent-search",
            ...
            "XAI_API_KEY": "未配置",
            "AGENT_SEARCH_LOG_DIR": "logs",
            "config_sources": {
                "XAI_API_KEY": "default",
                "AGENT_SEARCH_LOG_DIR": "default",
            },
    ...
    assert "# WolfHolo Agent Search Doctor" in out
    assert "Overall: OK" in out
    assert "Config dir source: `environment`" in out
    assert "Default config file: `C:/Users/example/AppData/Local/agent-search/config.json`" in out
    assert "Legacy Windows config file: `C:/Users/example/.config/agent-search/config.json`" in out
    assert "Legacy Windows config exists: OK" in out
    assert "AGENT_SEARCH_CONFIG_DIR: `C:/Users/example/AppData/Local/agent-search`" in out
...
def test_deep_alias_and_markdown_output(monkeypatch, capsys):
            ...
            "steps": [
                {
                    ...
                    "command": "agent-search fetch \"https://example.com\" --format markdown",
                }
            ],
    ...
    assert "agent-search fetch" in out
...
def test_search_timeout_respects_requested_format_and_exit_4(monkeypatch, capsys):
    ...
    assert "agent-search diagnose openai-compatible --format markdown" in out.out
    ...
    assert data["diagnose_command"] == "agent-search diagnose openai-compatible --format markdown"
...
def test_real_doctor_ignores_legacy_primary_env_and_returns_config_exit(monkeypatch, capsys):
    ...
    monkeypatch.setenv("AGENT_SEARCH_API_URL", "https://api.example.com/v1")
    monkeypatch.setenv("AGENT_SEARCH_API_KEY", secret)
    ...
    assert "AGENT_SEARCH_API_URL" not in data
    assert "AGENT_SEARCH_API_KEY" not in data
...
def test_model_set_returns_parameter_error(monkeypatch, capsys):
    def fake_set_model(model):
        return {
            ...
            "config_file": "C:/tmp/agent-search-config.json",
        }
...
def test_config_set_legacy_main_search_key_returns_parameter_error(monkeypatch, capsys):
    ...
    code = cli.main(["config", "set", "AGENT_SEARCH_API_KEY", "sk-test-secret"])
    data = json.loads(capsys.readouterr().out)

    assert code == cli.EXIT_PARAMETER_ERROR
    assert data["error_type"] == "parameter_error"
    assert "Unsupported config key: AGENT_SEARCH_API_KEY" in data["error"]
...
def test_smoke_markdown_and_content_are_human_readable(monkeypatch, capsys):
    async def fake_smoke(mode="mock"):
        return {
            ...
            "mode": mode,
            "failed_cases": [],
            "degraded_cases": ["web search fallback chain"],
            "cases": [
                {"name": "doctor minimum profile", "ok": True},
                {"name": "web search fallback chain", "ok": False, "severity": "degraded", "error": "HTTP 429"},
            ],
        }
    ...
    assert markdown_code == cli.EXIT_OK
    assert "# WolfHolo Agent Search Smoke" in markdown_out
    assert "web search fallback chain" in markdown_out
...
def test_config_markdown_and_content_are_masked_and_non_json(monkeypatch, capsys):
    ...
    assert "# WolfHolo Agent Search Config" in path_out
    assert "C:/tmp/config.json" in path_out
    assert "Config dir source: `environment`" in path_out
    assert "AGENT_SEARCH_CONFIG_DIR: `C:/tmp`" in path_out
...
def test_model_markdown_and_content_are_human_readable(monkeypatch, capsys):
    ...
    markdown_out = capsys.readouterr().out
    assert "# WolfHolo Agent Search Model" in markdown_out
...
def test_skills_status_reports_missing_and_update_writes_target(tmp_path, capsys):
    ...
    assert (tmp_path / ".codex" / "skills" / "agent-search-cli" / "SKILL.md").is_file()
...
def test_skills_unknown_target_returns_parameter_error(tmp_path, capsys):
    ...
    assert not (tmp_path / ".codex" / "skills" / "agent-search-cli").exists()
...
def test_setup_non_interactive_saves_values(monkeypatch, capsys):
    ...
    assert saved["AGENT_SEARCH_VALIDATION_LEVEL"] == "balanced"
    assert saved["AGENT_SEARCH_FALLBACK_MODE"] == "auto"
    assert saved["AGENT_SEARCH_MINIMUM_PROFILE"] == "standard"
...
def test_setup_non_interactive_installs_selected_skills_under_user_root_override(monkeypatch, tmp_path, capsys):
    ...
    assert (tmp_path / ".codex" / "skills" / "agent-search-cli" / "SKILL.md").is_file()
    assert (tmp_path / ".claude" / "skills" / "agent-search-cli" / "SKILL.md").is_file()
    assert (tmp_path / ".cursor" / "skills" / "agent-search-cli" / "SKILL.md").is_file()
...
def test_setup_non_interactive_installs_skill_under_home_by_default(monkeypatch, tmp_path, capsys):
    ...
    assert (fake_home / ".codex" / "skills" / "agent-search-cli" / "SKILL.md").is_file()
    assert (fake_home / ".hermes" / "skills" / "agent-search-cli" / "SKILL.md").is_file()
    assert not (tmp_path / "project" / ".codex" / "skills" / "agent-search-cli").exists()
    assert not (tmp_path / "project" / ".hermes" / "skills" / "agent-search-cli").exists()
...
def test_setup_guided_installs_tui_selected_skill_targets(monkeypatch, tmp_path, capsys):
    ...
    assert "Install the agent-search-cli skill" in captured.err
...
def test_setup_banner_falls_back_when_pyfiglet_unavailable(monkeypatch, capsys):
    ...
    assert "WolfHolo Agent Search" in captured.err
...
def test_skill_installer_parse_aliases_and_all(tmp_path):
    ...
    assert (tmp_path / "project" / ".codex" / "skills" / "agent-search-cli" / "SKILL.md").is_file()
...
def test_skill_installer_pi_target_uses_agent_skill_root(tmp_path):
    ...
    assert result["ok"] is True
    assert result["installed_count"] == 1
    assert Path(result["installed"][0]["path"]).as_posix().endswith(".pi/agent/skills/agent-search-cli")
    assert (tmp_path / "project" / ".pi" / "agent" / "skills" / "agent-search-cli" / "SKILL.md").is_file()
    assert not (tmp_path / "project" / ".pi" / "skills" / "agent-search-cli").exists()
...
def test_skill_installer_status_detects_stale_and_extra_files(tmp_path):
    ...
    dest = root / ".codex" / "skills" / "agent-search-cli"
...
def test_setup_guided_anysearch_optional_reinforcement_saves_vertical_search(monkeypatch, capsys):
    saved = {}
    answers = iter(["skip", "skip", "skip", "anysearch", "https://anysearch.example.com/mcp", "12", "n"])
    secrets = iter(["as-test-secret"])
    ...
    assert saved["ANYSEARCH_API_URL"] == "https://anysearch.example.com/mcp"
    assert saved["ANYSEARCH_API_KEY"] == "as-test-secret"
    assert saved["ANYSEARCH_TIMEOUT_SECONDS"] == "12"
    assert "vertical_search" in captured.err
    assert "as-test-secret" not in captured.out
    assert "as-test-secret" not in captured.err
...
def test_setup_guided_en_reports_missing_minimum(monkeypatch, capsys):
    ...
    assert "WolfHolo Agent Search setup wizard" in captured.err
...
def test_search_passes_routing_options(monkeypatch, capsys):
    ...
    assert code == cli.EXIT_OK
    assert captured == {"validation": "strict", "fallback": "off", "providers": "grok,tavily"}
    assert json.loads(capsys.readouterr().out)["content"] == "Answer"
...
def test_regression_uses_mock_smoke_when_packaged_tests_missing(monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli.Path, "resolve", lambda self: MissingPath("pkg/src/agent_search/cli.py"))
