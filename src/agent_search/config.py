...
class Config:
    _instance = None
    _SETUP_COMMAND = (
        "Run `agent-search setup`, or configure XAI_API_KEY and/or "
    ...
    _CONFIG_KEYS = {
        ...
        "AGENT_SEARCH_DEBUG",
        "AGENT_SEARCH_LOG_LEVEL",
        "AGENT_SEARCH_LOG_DIR",
        "AGENT_SEARCH_RETRY_MAX_ATTEMPTS",
        "AGENT_SEARCH_RETRY_MULTIPLIER",
        "AGENT_SEARCH_RETRY_MAX_WAIT",
        "AGENT_SEARCH_OUTPUT_CLEANUP",
        "AGENT_SEARCH_LOG_TO_FILE",
        "SSL_VERIFY",
    }
    ...
    def _default_config_dir() -> Path:
        if sys.platform.startswith("win"):
            ...
            if local_appdata:
                return Path(local_appdata).expanduser() / "agent-search"
        return Path.home() / ".config" / "agent-search"
    ...
    @staticmethod
    def _legacy_smart_search_windows_config_dir() -> Path:
        return Path.home() / ".config" / "smart-search"

    @staticmethod
    def _config_dir_override_value() -> str:
        return os.getenv("AGENT_SEARCH_CONFIG_DIR") or ""
    ...
    def _resolve_config_dir() -> tuple[Path, str]:
        env_dir = os.getenv("AGENT_SEARCH_CONFIG_DIR")
        if env_dir:
            return Path(env_dir).expanduser(), "environment"
        legacy_env_dir = os.getenv("SMART_SEARCH_CONFIG_DIR")
        if legacy_env_dir:
            legacy_path = Path(legacy_env_dir).expanduser()
            if (legacy_path / "config.json").exists():
                return legacy_path, "legacy_smart_search_environment"
        ...
        if sys.platform.startswith("win"):
            ...
            legacy_smart_dir = Config._legacy_smart_search_windows_config_dir()
            if not (default_dir / "config.json").exists() and (legacy_smart_dir / "config.json").exists():
                return legacy_smart_dir, "legacy_smart_search_windows_home"
    ...
    def _save_config_file(self, config_data: dict) -> None:
        try:
            ...
            hint = " (sandbox/CI 下可设 AGENT_SEARCH_CONFIG_DIR 指向可写目录)" if isinstance(e, PermissionError) else ""
    ...
    def config_path_info(self) -> dict:
        return {
            ...
            "legacy_smart_search_config_file": str(self._legacy_smart_search_windows_config_dir() / "config.json") if sys.platform.startswith("win") else "",
            "legacy_smart_search_config_exists": (self._legacy_smart_search_windows_config_dir() / "config.json").exists() if sys.platform.startswith("win") else False,
    ...
    def debug_enabled(self) -> bool:
        return (self._get_config_value("AGENT_SEARCH_DEBUG", "false") or "false").lower() in ("true", "1", "yes")

    @property
    def retry_max_attempts(self) -> int:
        return int(self._get_config_value("AGENT_SEARCH_RETRY_MAX_ATTEMPTS", "3") or "3")

    @property
    def retry_multiplier(self) -> float:
        return float(self._get_config_value("AGENT_SEARCH_RETRY_MULTIPLIER", "1") or "1")

    @property
    def retry_max_wait(self) -> int:
        return int(self._get_config_value("AGENT_SEARCH_RETRY_MAX_WAIT", "10") or "10")
    ...
    def log_level(self) -> str:
        return (self._get_config_value("AGENT_SEARCH_LOG_LEVEL", "INFO") or "INFO").upper()
    ...
    def log_dir_config_value(self) -> str:
        return self._get_config_value("AGENT_SEARCH_LOG_DIR", "logs") or "logs"
    ...
    def output_cleanup_enabled(self) -> bool:
        return (self._get_config_value("AGENT_SEARCH_OUTPUT_CLEANUP", "true") or "true").lower() in ("true", "1", "yes")

    @property
    def log_to_file_enabled(self) -> bool:
        return (self._get_config_value("AGENT_SEARCH_LOG_TO_FILE", "false") or "false").lower() in ("true", "1", "yes")
    ...
    def get_config_info(self) -> dict:
        ...
        return {
            ...
            "AGENT_SEARCH_VALIDATION_LEVEL": validation_level,
            "AGENT_SEARCH_FALLBACK_MODE": fallback_mode,
            "AGENT_SEARCH_MINIMUM_PROFILE": minimum_profile,
            "AGENT_SEARCH_DEBUG": self.debug_enabled,
            "AGENT_SEARCH_LOG_LEVEL": self.log_level,
            "AGENT_SEARCH_LOG_DIR": self.log_dir_config_value,
            "AGENT_SEARCH_RETRY_MAX_ATTEMPTS": self.retry_max_attempts,
            "AGENT_SEARCH_RETRY_MULTIPLIER": self.retry_multiplier,
            "AGENT_SEARCH_RETRY_MAX_WAIT": self.retry_max_wait,
            ...
            "AGENT_SEARCH_OUTPUT_CLEANUP": self.output_cleanup_enabled,
            "AGENT_SEARCH_LOG_TO_FILE": self.log_to_file_enabled,
