...
from . import service
from .branding import PACKAGE_NAME
...
def _search_timeout_result(query: str, timeout: float, search_kwargs: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "model": model,
        "stream": stream,
        "diagnose_command": "agent-search diagnose openai-compatible --format markdown",
        "recommendation": "Run `agent-search diagnose openai-compatible --format markdown` to check whether OpenAI-compatible stream/no-stream search requests are hanging upstream.",
    }
...
def _format_doctor_markdown(data: dict[str, Any]) -> str:
    lines = [
        ...
        f"Log dir config value: `{data.get('log_dir_config_value', data.get('AGENT_SEARCH_LOG_DIR', ''))}`",
    ...
    if data.get("config_dir_override_value"):
        lines.append(f"AGENT_SEARCH_CONFIG_DIR: `{data.get('config_dir_override_value')}`")
        ...
        if data.get("config_dir_source") == "environment" and data.get("config_dir_override_matches_default"):
            lines.append(
                "The active config path comes from `AGENT_SEARCH_CONFIG_DIR`, but that override matches the current Windows default path."
            )
    if data.get("config_dir_source") == "legacy_windows_home":
        lines.append(
            "Active config is using the old Windows `~\\.config\\agent-search` location because the new default file does not exist."
        )
...
def _format_diagnose_markdown(data: dict[str, Any]) -> str:
    lines = [
        "# WolfHolo Agent Search Diagnose",
        "",
...
def _format_config_markdown(data: dict[str, Any]) -> str:
    lines = ["# WolfHolo Agent Search Config", "", f"Status: {_status_label(data.get('ok'))}"]
...
def _format_model_markdown(data: dict[str, Any]) -> str:
    lines = ["# WolfHolo Agent Search Model", "", f"Status: {_status_label(data.get('ok'))}"]
    rows = []
...
def _format_setup_markdown(data: dict[str, Any]) -> str:
    lines = ["# WolfHolo Agent Search Setup", "", f"Status: {_status_label(data.get('ok'))}"]
    if data.get("config_file"):
        lines.append(f"Config file: `{data.get('config_file')}`")
...
def _format_skills_markdown(data: dict[str, Any]) -> str:
    lines = ["# WolfHolo Agent Search Skills", "", f"Status: {_status_label(data.get('ok'))}"]
    if data.get("root"):
...
def _format_markdown(command: str, data: dict[str, Any]) -> str:
    if command == "search":
        if not data.get("ok", False) and (data.get("error") or data.get("error_type")):
            lines = ["# WolfHolo Agent Search Search", ""]
            if data.get("query"):
                lines.append(f"Query: `{data.get('query')}`")
...
def _agent_search_banner_text() -> str:
    try:
        import pyfiglet

        banner = pyfiglet.figlet_format("WolfHolo Agent Search", font="slant")
        return banner.rstrip()
    except Exception:
        return _STATIC_AGENT_SEARCH_BANNER


def _write_setup_banner(lang: str) -> None:
    banner = _agent_search_banner_text()
...
def _write_panel(text: str, lang: str) -> None:
    ...
    title = _t(lang, "WolfHolo Agent Search 配置", "WolfHolo Agent Search Setup")
...
def _prompt_skill_targets(lang: str) -> list[str]:
    _write_stderr(
        _t(
            lang,
            "\n[可选] 安装 agent-search-cli skill\n用途: 让本机全局 AI 工具知道优先调用 agent-search CLI。\n提示: 只安装 WolfHolo Agent Search skill；不会初始化 Trellis，也不会生成 hooks、agents 或 commands。\n",
            "\n[Optional] Install the agent-search-cli skill\nPurpose: teach user-level AI tools on this machine to call the agent-search CLI first.\nNote: this only installs the WolfHolo Agent Search skill; it does not initialize Trellis or generate hooks, agents, or commands.\n",
        )
    )
...
def _prompt_optional_enhancements(values: dict[str, str], current: dict[str, str], lang: str) -> None:
    _write_stderr(
        _t(
            lang,
            "\n[可选增强] vertical_search 垂直搜索\n用途: AnySearch 实验性垂直/结构化来源检索；不进入 standard 最低配置。\n",
            "\n[Optional] vertical_search\nPurpose: experimental AnySearch vertical/structured source discovery; not part of the standard minimum profile.\n",
        )
    )
    default_selected = ["anysearch"] if current.get("ANYSEARCH_API_URL") or current.get("ANYSEARCH_API_KEY") else []
    selected = _prompt_provider_multi_select(
        _t(lang, "选择可选 vertical_search 增强", "Choose optional vertical_search reinforcement"),
        ["anysearch"],
        default_selected,
        lang,
    )
    if "anysearch" in selected:
        values["ANYSEARCH_API_URL"] = _prompt_value("ANYSEARCH_API_URL", "AnySearch API URL", current.get("ANYSEARCH_API_URL", ""), optional=True, lang=lang)
        values["ANYSEARCH_API_KEY"] = _prompt_value("ANYSEARCH_API_KEY", "AnySearch API key", current.get("ANYSEARCH_API_KEY", ""), optional=True, lang=lang)
        values["ANYSEARCH_TIMEOUT_SECONDS"] = _prompt_value(
            "ANYSEARCH_TIMEOUT_SECONDS",
            _t(lang, "AnySearch 超时秒数", "AnySearch timeout seconds"),
            current.get("ANYSEARCH_TIMEOUT_SECONDS", ""),
            optional=True,
            lang=lang,
        )
    if _prompt_yes_no(_t(lang, "是否调整验证/兜底默认值?", "Adjust validation/fallback defaults?"), default=False):
        values["AGENT_SEARCH_VALIDATION_LEVEL"] = _prompt_value(
            "AGENT_SEARCH_VALIDATION_LEVEL",
            _t(lang, "验证强度 (fast/balanced/strict)", "Validation level (fast/balanced/strict)"),
            current.get("AGENT_SEARCH_VALIDATION_LEVEL", ""),
            optional=True,
            lang=lang,
        )
        values["AGENT_SEARCH_FALLBACK_MODE"] = _prompt_value(
            "AGENT_SEARCH_FALLBACK_MODE",
            _t(lang, "兜底模式 (auto/off)", "Fallback mode (auto/off)"),
            current.get("AGENT_SEARCH_FALLBACK_MODE", ""),
            optional=True,
            lang=lang,
        )
        values["AGENT_SEARCH_MINIMUM_PROFILE"] = _prompt_value(
            "AGENT_SEARCH_MINIMUM_PROFILE",
            _t(lang, "最低配置门槛 (standard/off)", "Minimum profile (standard/off)"),
            current.get("AGENT_SEARCH_MINIMUM_PROFILE", ""),
            optional=True,
            lang=lang,
        )
...
def _write_setup_keep_note(lang: str) -> None:
    _write_stderr(
        _t(
            lang,
            "\n提示: setup 不会删除旧配置；删除请运行 `agent-search config unset KEY`。\n",
            "\nNote: setup does not delete saved values; use `agent-search config unset KEY` to remove one.\n",
        )
    )
...
def _write_setup_examples(lang: str) -> None:
    _write_stderr(
        _t(
            ...
            "  key 都填你自己控制台里的；Firecrawl / AnySearch 可以之后再补。\n",
            ...
            "  Use keys from your own provider consoles. Firecrawl / AnySearch can be added later.\n",
        )
    )
...
def _run_advanced_setup_prompts(values: dict[str, str], current: dict[str, str], lang: str) -> None:
    ...
    prompts = [
        ...
        ("AGENT_SEARCH_VALIDATION_LEVEL", "Validation level (fast/balanced/strict)", True),
        ("AGENT_SEARCH_FALLBACK_MODE", "Fallback mode (auto/off)", True),
        ("AGENT_SEARCH_MINIMUM_PROFILE", "Minimum profile (standard/off)", True),
...
def _run_setup(args: argparse.Namespace) -> int:
    ...
    values = {
        ...
        "AGENT_SEARCH_VALIDATION_LEVEL": args.validation_level,
        "AGENT_SEARCH_FALLBACK_MODE": args.fallback_mode,
        "AGENT_SEARCH_MINIMUM_PROFILE": args.minimum_profile,
...
def build_parser() -> argparse.ArgumentParser:
    parser = SmartSearchArgumentParser(
        prog="agent-search",
        description="WolfHolo Agent Search CLI for AI-agent web research.",
    )
    ...
    skills_parser = sub.add_parser(
        ...
        help="Inspect or update installed agent-search-cli skills.",
    )
    ...
    setup_parser.add_argument("--skip-skills", action="store_true", help="Skip user-level agent-search-cli skill installation.")
    setup_parser.add_argument(
        "--install-skills",
        default="",
        help="Comma-separated AI tool targets for agent-search-cli skill installation, e.g. codex,claude,cursor,hermes.",
    )
    setup_parser.add_argument(
        "--skills-root",
        default="",
    ...
    setup_parser.add_argument("--validation-level", default="", help="Save AGENT_SEARCH_VALIDATION_LEVEL.")
    setup_parser.add_argument("--fallback-mode", default="", help="Save AGENT_SEARCH_FALLBACK_MODE.")
    setup_parser.add_argument("--minimum-profile", default="", help="Save AGENT_SEARCH_MINIMUM_PROFILE.")
    ...
    config_parser = sub.add_parser(
        "config", aliases=COMMAND_ALIASES["config"], help="Read or edit the local WolfHolo Agent Search config file."
    )
