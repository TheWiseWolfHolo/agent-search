...
OPENAI_COMPATIBLE_DIAGNOSE_COMMAND = "agent-search diagnose openai-compatible --format markdown"
...
WEB_SEARCH_FALLBACK_CHAIN = ["tavily", "firecrawl"]
DOCS_SEARCH_FALLBACK_CHAIN = ["context7", "exa"]
WEB_FETCH_FALLBACK_CHAIN = ["tavily", "firecrawl"]
VERTICAL_SEARCH_FALLBACK_CHAIN = ["anysearch"]
...
def build_deep_research_plan(query: str, budget: str = "standard", evidence_dir: str = "") -> dict[str, Any]:
    ...
    def command_search(q: str, extra_sources: int = 2) -> str:
        return f"agent-search search {_quote_arg(q)} --validation balanced --extra-sources {extra_sources} --format json --output {_quote_arg(_path_join(evidence_root, next_filename('search.json')))}"

    def command_exa(q: str) -> str:
        return f"agent-search exa-search {_quote_arg(q)} --num-results 5 --format json --output {_quote_arg(_path_join(evidence_root, next_filename('exa.json')))}"

    def command_web(q: str, extra_sources: int = 5) -> str:
        return f"agent-search search {_quote_arg(q)} --validation balanced --extra-sources {extra_sources} --format json --output {_quote_arg(_path_join(evidence_root, next_filename('web.json')))}"

    def command_fetch(target: str = "<key-url>") -> str:
        return f"agent-search fetch {_quote_arg(target)} --format markdown --output {_quote_arg(_path_join(evidence_root, next_filename('fetch.md')))}"
    ...
    if known_url:
        ...
        add_step("sq1", "fetch", "fetch user supplied URL first", f"agent-search fetch {_quote_arg(url)} --format markdown --output {_quote_arg(_path_join(evidence_root, '01-fetch.md'))}", "01-fetch.md")
        add_step("sq2", "exa-similar", "find adjacent sources from the provided URL", f"agent-search exa-similar {_quote_arg(url)} --num-results 5 --format json --output {_quote_arg(_path_join(evidence_root, '02-similar.json'))}", "02-similar.json")
        ...
        if docs_intent:
            ...
            add_step(
                ...
                f"agent-search context7-library {_quote_arg(library_hint)} {_quote_arg(question)} --format json --output {_quote_arg(_path_join(evidence_root, next_filename('context7-library.json')))}",
            ...
            add_step(
                ...
                f"agent-search context7-docs {_quote_arg('<library_id>')} {_quote_arg(question)} --format json --output {_quote_arg(_path_join(evidence_root, next_filename('context7-docs.json')))}",
        ...
        if recency_requirement != "none" or locale_domain_scope == "china":
            ...
            decomposition.append(
                _deep_subquestion(
                    ...
                    "Current or locale-scoped prompts benefit from web evidence reinforcement.",
                    ["current_or_locale_source_discovery"],
                )
            )
            capability_plan.append(
                _deep_capability("current_or_locale_source_discovery", ["search"], "Reinforce current or locale-specific web evidence through the web_search chain.")
            )
            add_step(sub_id, "search", "current or locale-specific source discovery", command_web(question), f"{len(steps) + 1:02d}-web.json")
        ...
        if cross_validation_need == "high":
            ...
            if recency_requirement != "none" or locale_domain_scope == "china" or zh_current_intent:
                ...
                if not any(step["tool"] == "search" and "current or locale-specific" in step.get("purpose", "") for step in steps):
                    add_step(target_subquestion, "search", "current or locale-specific cross-source discovery", command_web(question), next_filename("web.json"))
            elif docs_intent:
    ...
    if budget == "quick" and len(steps) > 4:
        ...
        if not any(step["tool"] == "fetch" for step in limited_steps):
            ...
            if first_fetch:
                ...
                first_fetch["command"] = f"agent-search fetch {_quote_arg('<key-url>')} --format markdown --output {_quote_arg(fetch_path)}"
...
def get_capability_status() -> dict[str, Any]:
    ...
    status = {
        ...
        "web_search": {
            ...
            "fallback_chain": WEB_SEARCH_FALLBACK_CHAIN,
        },
        "docs_search": {
            "configured": [
                name
            ...
            "fallback_chain": DOCS_SEARCH_FALLBACK_CHAIN,
        },
        "web_fetch": {
            "configured": [
                name
            ...
            "fallback_chain": WEB_FETCH_FALLBACK_CHAIN,
        },
        "vertical_search": {
            "configured": ["anysearch"] if config.anysearch_api_key else [],
            "fallback_chain": VERTICAL_SEARCH_FALLBACK_CHAIN,
            "experimental": True,
        },
    }
...
async def call_tavily_map(
    ...
    if not api_key:
        return {
            ...
            "error": "TAVILY_API_KEY 未配置。请运行 `agent-search setup`，或使用 `agent-search config set TAVILY_API_KEY <key>`。",
        }
...
async def exa_search(
    ...
    if not api_key:
        return {
            "ok": False,
            "error_type": "config_error",
            "error": "EXA_API_KEY 未配置。请运行 `agent-search setup`，或使用 `agent-search config set EXA_API_KEY <key>`。",
        }
...
async def context7_library(name: str, query: str = "") -> dict[str, Any]:
    ...
    if not api_key:
        return {
            "ok": False,
            "error_type": "config_error",
            "error": "CONTEXT7_API_KEY 未配置。请运行 `agent-search setup`，或使用 `agent-search config set CONTEXT7_API_KEY <key>`。",
        }
...
def _openai_compatible_diagnosis(quick: dict[str, Any], no_stream: dict[str, Any], stream: dict[str, Any]) -> tuple[bool, str, str]:
    ...
    if stream_ok and not no_stream_ok:
        return (
            False,
            "非流式请求不稳定，流式请求可用。",
            "建议设置 `OPENAI_COMPATIBLE_STREAM=true`，或临时使用 `agent-search search ... --stream`。",
        )
    if no_stream_ok and not stream_ok:
        return (
            False,
            "流式请求不稳定，非流式请求可用。",
            "建议设置 `OPENAI_COMPATIBLE_STREAM=false`，或临时使用 `agent-search search ... --no-stream`。",
        )
    if quick_ok and search_timeout:
        return (
            False,
            "小请求能通，但真实 search 形态超时。",
            "这通常是上游模型或中转站在处理 agent-search 的完整 prompt 时卡住；建议换模型/中转，或把本诊断报告贴给维护者。",
        )
    if quick_ok:
        return (
            False,
            "小请求能通，但真实 search 形态失败。",
            "这更像上游模型/中转站对 agent-search 请求形态不兼容；建议换模型/中转，或把本诊断报告贴给维护者。",
        )
    return (
        False,
...
async def _probe_openai_compatible_search_shape(
    ...
    *,
    ...
    headers = {
        ...
        "User-Agent": "agent-search/diagnose",
    }
        ...
        result.update(
            {
                ...
                "recommendation": "请先运行 `agent-search setup`，或用 `agent-search config set` 填好缺失项。",
                "missing": missing,
                "elapsed_ms": _elapsed_ms(start),
            }
        )
...
def set_model(model: str) -> dict[str, Any]:
    return {
        ...
        "error": (
            "The legacy default model command was removed. Use `agent-search config set XAI_MODEL <model>` "
            "or `agent-search config set OPENAI_COMPATIBLE_MODEL <model>`."
        ),
        "config_file": str(config.config_file),
    }
...
async def _smoke_mock(start: float) -> dict[str, Any]:
    ...
    minimum_status = {
        ...
        "web_search": {"configured": ["tavily"], "fallback_chain": WEB_SEARCH_FALLBACK_CHAIN, "ok": True},
        "docs_search": {"configured": ["context7"], "fallback_chain": DOCS_SEARCH_FALLBACK_CHAIN, "ok": True},
        "web_fetch": {"configured": ["tavily"], "fallback_chain": WEB_FETCH_FALLBACK_CHAIN, "ok": True},
        "vertical_search": {"configured": [], "fallback_chain": VERTICAL_SEARCH_FALLBACK_CHAIN, "ok": False, "experimental": True},
    }
    ...
    missing_minimum = _minimum_profile_result(
        ...
        {
            **minimum_status,
            "docs_search": {"configured": [], "fallback_chain": DOCS_SEARCH_FALLBACK_CHAIN, "ok": False},
        },
    )
    cases.append(
        _case(
    ...
    cases.append(_case("search local current intent uses web reinforcement", zh_route["zh_current_intent"], {"routing_decision": zh_route}))
    ...
    market_plan = build_deep_research_plan("深度搜索一下最近的比特币行情", evidence_dir=r"C:\tmp\agent-search-evidence\market")
    ...
    docs_plan = build_deep_research_plan("深度调研 React useEffect 最新文档", evidence_dir=r"C:\tmp\agent-search-evidence\docs")
    ...
    claim_plan = build_deep_research_plan("帮我核验这个说法是真是假", evidence_dir=r"C:\tmp\agent-search-evidence\claim")
    cases.append(
        _case(
    ...
    url_first_plan = build_deep_research_plan("深度调研 https://example.com/source", evidence_dir=r"C:\tmp\agent-search-evidence\url")
    cases.append(
        _case(
    ...
    normal_prompt = "搜索一下 agent-search 怎么安装"
    cases.append(
        _case(
...
async def _smoke_live(start: float) -> dict[str, Any]:
    ...
    if capability_status.get("web_search", {}).get("configured"):
        web_sources, web_attempts = await _run_web_search_fallback("test", count=1)
        web_ok = bool(web_sources)
        ...
        cases.append(
            _case(
                ...
                web_ok,
                {
                    "severity": "" if web_ok else ("degraded" if web_fallback_available else "critical"),
                    "fallback_available": web_fallback_available,
                    "provider_attempts": web_attempts,
                },
            )
        )
    else:
        cases.append(_case("web search fallback chain", True, {"skipped": "no web search providers configured"}))
