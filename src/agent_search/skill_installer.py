...
SKILL_NAME = "agent-search-cli"
PACKAGE_ROOT_ENV = "AGENT_SEARCH_PACKAGE_ROOT"
...
def _resource_skill_root() -> Any:
    try:
        root = resources.files("agent_search").joinpath("assets", "skills", SKILL_NAME)
        if root.is_dir():
            return root
...
def _filesystem_skill_root() -> Path | None:
    ...
    if package_root:
        ...
        candidates.extend([
            base / "src" / "agent_search" / "assets" / "skills" / SKILL_NAME,
            base / "skills" / SKILL_NAME,
        ])

    repo_root = Path(__file__).resolve().parents[2]
    candidates.extend([
        repo_root / "src" / "agent_search" / "assets" / "skills" / SKILL_NAME,
...
def _load_skill_files(source_root: Path | None = None) -> list[tuple[str, bytes]]:
    ...
    raise SkillInstallError("Bundled agent-search-cli skill files were not found.")
