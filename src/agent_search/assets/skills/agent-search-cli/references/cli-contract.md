# WolfHolo Agent Search CLI Contract

## Entrypoints

- `agent-search` is the primary CLI.
- `agent-search --version`, `agent-search --v`, and `agent-search -v` print the installed version and exit with code `0`.
- `agent-search` should resolve from the user's PATH.
- This bundled skill is maintained with the `TheWiseWolfHolo/agent-search` repository.
- Private API keys should be saved with `agent-search setup` or `agent-search config set`.
...
- On Windows with mise, the managed package name is `npm:@thewisewolfholo/agent-search`; the executable remains `agent-search`. Diagnose mise managed installs with `mise ls "npm:@thewisewolfholo/agent-search"` and `mise which agent-search` (the bare name `agent-search` is the bin, not a mise tool identifier).
- On Windows, the default config file is `%LOCALAPPDATA%\agent-search\config.json`. Linux/macOS default to `~/.config/agent-search/config.json`.
- `AGENT_SEARCH_CONFIG_DIR` is an advanced override for CI, containers, sandboxes, or portable installs. The CLI uses it for config and relative logs and skips default-directory selection.
- Earlier Windows source defaults used `~\.config\agent-search\config.json`, while some installs were already pinned to `%LOCALAPPDATA%\agent-search` through `AGENT_SEARCH_CONFIG_DIR`. If the new Windows default file is missing but the old file exists, the active config source is `legacy_windows_home` so upgrades do not silently lose configuration. Diagnostics must expose the override value and whether it matches the current default.
...
- `agent-search anysearch-domains [DOMAIN] [--format json|markdown|content] [--output PATH]`
- `agent-search anysearch-search QUERY [--domain DOMAIN] [--sub-domain SUB_DOMAIN] [--max-results N] [--format json|markdown|content] [--output PATH]`
- `agent-search anysearch-extract URL [--max-length N] [--format json|markdown|content] [--output PATH]`
- `agent-search anysearch-batch QUERY... [--max-results N] [--format json|markdown|content] [--output PATH]`
- `agent-search context7-library NAME [QUERY] [--format json|markdown|content] [--output PATH]`
- `agent-search context7-docs LIBRARY_ID QUERY [--format json|markdown|content] [--output PATH]`
- `agent-search deep QUERY [--budget quick|standard|deep] [--evidence-dir PATH] [--format json|markdown|content] [--output PATH]`
...
- `agent-search config unset KEY [--format json|markdown|content] [--output PATH]`
- `agent-search model set MODEL [--format json|markdown|content] [--output PATH]`
- `agent-search model current [--format json|markdown|content] [--output PATH]`
- `agent-search regression`
- `agent-search smoke [--mode mock|live] [--mock] [--live] [--format json|markdown|content] [--output PATH]`
...
| --- | --- |
| `agent-search --version` | `agent-search --v`, `agent-search -v` |
| `search` | `s` |
| `fetch` | `f` |
| `map` | `m` |
| `exa-search` | `exa`, `x` |
...
Web-source reinforcement setup:
...
Before execution, the skill should call `agent-search deep "question" --format json` to create a `research_plan` JSON artifact. Required fields are:
...
2. Call `agent-search deep "question" --format json` to generate `intent_signals`, `decomposition`, and `capability_plan` instead of selecting a fixed topic recipe.
3. Use planned `agent-search search ... --validation balanced --extra-sources 1..3` steps for broad discovery.
4. Add planned `search --extra-sources` for current or supplementary web discovery, `context7-library` plus `context7-docs` for docs/API/library topics, `exa-search` for official/trusted-domain or paper discovery, `exa-similar` for URL-neighbor discovery, or `map` only when the capability boundary matches the intent.
...
- Default `agent-search setup` shows a WolfHolo Agent Search ASCII banner, asks for `zh`
  or `en`, offers user-level `agent-search-cli` skill installation, then
...
- The grouped wizard should use an arrow-key / Space / Enter selector when the
  ...
  and tests.
- Skill installation installs the bundled `agent-search-cli` skill into
...
- Skill targets are `codex`, `claude`, `cursor`, `opencode`, `copilot`,
  ...
  should omit it.
- `agent-search skills status --targets codex,claude,cursor,hermes --format json`
...
- `agent-search skills update --targets codex,claude,cursor,hermes --format json`
  overwrites the managed bundled `agent-search-cli` files for selected
  targets. `agent-search skills update --all --format json` selects every
  ...
  `skills status`.
- `agent-search setup --non-interactive --install-skills codex` remains the
...
- Unchecking a configured provider must not delete existing config values; use
  `agent-search config unset KEY` for deletion.
...
- Agents should retry up to 3 total attempts with `agent-search search ... --timeout 180 --extra-sources 1 --format json --output PATH`, waiting about 5 seconds between attempts and stopping as soon as the saved JSON has `"ok": true`.
...
- `AGENT_SEARCH_RETRY_*` settings are not the contract for this path; the visible CLI result is the contract.
...
- `setup` and `config` read/write the local WolfHolo Agent Search config file and do not call providers.
...
- Use `search --extra-sources` for current, domain-filtered, or supplementary web-source discovery when Tavily or Firecrawl is configured.
...
Run `agent-search regression` before considering CLI or skill changes complete.
...
- Always read back npm before and after publishing with `npm view @thewisewolfholo/agent-search versions --json` and `npm view @thewisewolfholo/agent-search dist-tags --json`. A test release must leave `latest` on the stable version and move only `next` or the explicitly supplied non-`latest` tag.
...
- Local verification after a test release must use an exact install target, such as `mise use -g "npm:@thewisewolfholo/agent-search@0.1.10-beta.3" -y --pin`, followed by `mise reshim`, `where.exe agent-search`, `agent-search --version`, packaged `agent-search regression`, and `agent-search smoke --mock --format json`. Also pipe a non-ASCII JSON command such as `agent-search deep "深度搜索一下最近的比特币行情" --format json | ConvertFrom-Json` to verify the Windows npm/mise wrapper is emitting UTF-8 JSON, not locale-encoded bytes.

## Tool Policy

Web research through this skill should use `agent-search` CLI. If the CLI is unavailable, report the blocker and recovery steps instead of silently falling back to another web-search route.
